...
void st_wake_up()
{
  ...
  // Enable stepper drivers.
	...
	#ifdef ENABLE_MIRROR_A_AXIS
      STEPPER_DISABLE_PORT(3) |= (1 << STEPPER_DISABLE_BIT(3));
	#endif

	#ifdef ENABLE_MIRROR_T_AXIS
...
#ifdef ENABLE_MIRROR_A_AXIS
	#ifdef A_AXIS_INVERT
    if((DIRECTION_PORT(A_AXIS_MIRROR) & (1 << DIRECTION_BIT(A_AXIS_MIRROR))) == 0)	//if this port is low
	#else
    	if((DIRECTION_PORT(A_AXIS_MIRROR) & (1 << DIRECTION_BIT(A_AXIS_MIRROR))) > 0)	//if this port is high
	#endif
    		DIRECTION_PORT(3) |= (1 << DIRECTION_BIT(3));	//set
    	else
    		DIRECTION_PORT(3) &= ~(1 << DIRECTION_BIT(3));	//clear
#endif

#ifdef ENABLE_MIRROR_T_AXIS
	#ifdef T_AXIS_INVERT
	if((DIRECTION_PORT(T_AXIS_MIRROR) & (1 << DIRECTION_BIT(T_AXIS_MIRROR))) == 0)	//if this port is low
	#else
		if((DIRECTION_PORT(T_AXIS_MIRROR) & (1 << DIRECTION_BIT(T_AXIS_MIRROR))) > 0)	//if this port is high
	#endif
			DIRECTION_PORT(4) |= (1 << DIRECTION_BIT(4));	//set
		else
			DIRECTION_PORT(4) &= ~(1 << DIRECTION_BIT(4));	//clear
#endif
...
#ifdef ENABLE_MIRROR_A_AXIS
      if((STEP_PORT(A_AXIS_MIRROR) & (1 << STEP_BIT(A_AXIS_MIRROR))) > 0)	//if this port is high
          STEP_PORT(3) |= (1 << STEP_BIT(3));	//set
      else
          STEP_PORT(3) &= ~(1 << STEP_BIT(3));	//clear
#endif

#ifdef ENABLE_MIRROR_T_AXIS
      if((STEP_PORT(T_AXIS_MIRROR) & (1 << STEP_BIT(T_AXIS_MIRROR))) > 0)	//if this port is high
          STEP_PORT(4) |= (1 << STEP_BIT(4));	//set
      else
          STEP_PORT(4) &= ~(1 << STEP_BIT(4));	//clear
#endif
